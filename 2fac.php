...
<body>
    ...
    if (isset($_POST['code'])) {
        ...
        if ($row['vpin'] == md5($code)) {
            // echo " activated";
            $sql = "UPDATE userdata SET status = 'active' WHERE token = '{$token}'";
            if ($result = mysqli_query($conn, $sql)) {
                echo '<div class="container mx-auto" style="justify-content: center;">
                        <div class="row mx-auto" style="justify-content: center;">
                            <div class="mx-auto" style="justify-content: center;">
                            <div class="alert alert-success"><em>Account Activated. Redirecting in 5 seconds</em></div>
                            </div>
                        </div>
                </div>';
                // echo '<h1 class= "display-4 text-center">Account Activated</h1>';
                // echo '<h1 class= "display-4 text-center">Log in to use your acccount. Redirecting in 3 seconds.</h1>';
